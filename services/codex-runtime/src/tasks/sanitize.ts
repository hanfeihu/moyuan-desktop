...
export function runtimeFailureDiagnostic(items: CodexTask['transcript'] | string) {
  ...
  if (/timeout|timed out|超时|模型响应超时/i.test(text)) {
    const hasLargeScan = /node_modules|dist|release|\.git|package-lock|find .*type f|grep -n|cat .*index\./i.test(text)
    if (hasLargeScan) {
